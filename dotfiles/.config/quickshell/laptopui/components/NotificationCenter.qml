...
Item {
    ...
    onOpenChanged: if (open) NotificationState.clearUnread()
    Connections {
        target: NotificationState.server
        function onNotification(notification) {
            if (!SettingsState.dnd && !notification.transient) toast.show(notification)
        }
    }
            ...
            Rectangle {
                ...
                ColumnLayout { anchors.fill: parent; anchors.margins: 16; spacing: 10
                    RowLayout { Layout.fillWidth: true
                        ...
                        Text { text: "Clear"; color: Theme.accent; font.family: Theme.fontFamily; font.pixelSize: 11; MouseArea { anchors.fill: parent; onClicked: NotificationState.clearHistory() } }
                    }
                    Rectangle { Layout.fillWidth: true; height: 1; color: Theme.surfaceHover }
                    ListView { id: history; Layout.fillWidth: true; Layout.fillHeight: true; spacing: 7; clip: true; model: NotificationState.server.trackedNotifications
                        ...
                        delegate: Rectangle { required property var modelData; width: history.width; height: actionList.count > 0 ? 104 : 76; radius: 11; color: Theme.surface
                            Column { anchors.fill: parent; anchors.margins: 11; spacing: 4
                                Text { text: (modelData.appName || "System") + " · " + NotificationState.timeLabel(modelData); color: Theme.accent; font.family: Theme.fontFamily; font.pixelSize: 10 }
